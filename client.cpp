...
void registration(connection* c, std::string message);
void registrationLogEmail(connection* c, std::string message); 
void registrationPassword (connection* c, std::string message);
void checkLogEmail (connection* c, std::string message);
...
void recvMessageFromServer(connection* c, std::string message)
{
	...
	{      std::cout << "Hello, for registration enter 1, to sign in enter 2, to quit enter 3" << std::endl;
		...
		if (1 == action)
		{
			...
			c->setRecvMessageCallback(registration);
		}
	}
}
		void registration(connection* c, std::string message)
		{	
			...
			c->setRecvMessageCallback(registrationLogEmail);
		}
		void registrationLogEmail(connection* c, std::string message)
		{
			...
			c->setRecvMessageCallback(registrationPassword);
		}
		...
		void registrationPassword (connection* c, std::string message)
		{
			...
			c->setRecvMessageCallback(checkLogEmail);
